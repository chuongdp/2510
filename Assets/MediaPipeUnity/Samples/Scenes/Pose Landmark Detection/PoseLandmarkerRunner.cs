...
namespace Mediapipe.Unity.Sample.PoseLandmarkDetection
{
  ...
  {
    public static event Action<PoseLandmarkerResult> OnPoseResult; // added: public event for external consumers
        ...
        switch (taskApi.runningMode)
        {
          case Tasks.Vision.Core.RunningMode.IMAGE:
            if (taskApi.TryDetect(image, imageProcessingOptions, ref result))
            {
              ...
              try { OnPoseResult?.Invoke(result); } catch (Exception e) { Debug.LogException(e); }
              // Debug log: landmarks info (throttle)
              _debugLogCounter++;
              if ((_debugLogCounter % 15) == 0)
              {
                var count = result.poseLandmarks == null ? 0 : result.poseLandmarks.Count;
                if (count > 0)
                {
                  ...
                  {
                    var p0 = lms[0];
                    Debug.Log($"[PoseLandmarkerRunner] IMAGE detected. count={count} nose=({p0.x:F3},{p0.y:F3})");
                  }
                  else
                  {
                    Debug.Log($"[PoseLandmarkerRunner] IMAGE detected. count={count} but empty landmarks list");
                  }
                }
                else
                {
                  Debug.Log("[PoseLandmarkerRunner] IMAGE detected but no pose landmarks");
                }
              }
            }
            else
            ...
            if (taskApi.TryDetectForVideo(image, GetCurrentTimestampMillisec(), imageProcessingOptions, ref result))
            {
              ...
              if ((_debugLogCounter % 15) == 0)
              {
                ...
                if (count > 0)
                {
                  ...
                  {
                    var p0 = lms[0];
                    Debug.Log($"[PoseLandmarkerRunner] VIDEO detected. count={count} nose=({p0.x:F3},{p0.y:F3})");
                  }
                  else
                  {
                    Debug.Log($"[PoseLandmarkerRunner] VIDEO detected. count={count} but empty landmarks list");
                  }
                }
                else
                {
                  Debug.Log("[PoseLandmarkerRunner] VIDEO detected but no pose landmarks");
                }
              }
            }
            else
    ...
    private void OnPoseLandmarkDetectionOutput(PoseLandmarkerResult result, Image image, long timestamp)
    {
      ...
      {
        var count = result.poseLandmarks == null ? 0 : result.poseLandmarks.Count;
        Debug.Log($"[PoseLandmarkerRunner] Pose callback fired. count={count}");
      }
    }
